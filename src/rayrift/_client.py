...
from ._exceptions import RayriftError, APIStatusError
...
if TYPE_CHECKING:
    from .resources import search, folders, documents
...
ENVIRONMENTS: Dict[str, str] = {
    "production": "https://api.rayrift.com",
    "local": "http://localhost:3000",
}


class Rayrift(SyncAPIClient):
    # client options
    api_key: str
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Rayrift client instance.

        This automatically infers the `api_key` argument from the `RAYRIFT_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("RAYRIFT_API_KEY")
        if api_key is None:
            raise RayriftError(
                "The api_key client option must be set either by passing api_key to the client or by setting the RAYRIFT_API_KEY environment variable"
            )
        self.api_key = api_key

        self._environment = environment

        base_url_env = os.environ.get("RAYRIFT_BASE_URL")
        if is_given(base_url) and base_url is not None:
            ...
            if base_url_env and base_url is not None:
                raise ValueError(
                    "Ambiguous URL; The `RAYRIFT_BASE_URL` env var and the `environment` argument are given. If you want to use the environment, you must pass base_url=None",
                )
    ...
    @cached_property
    def with_raw_response(self) -> RayriftWithRawResponse:
        return RayriftWithRawResponse(self)

    @cached_property
    def with_streaming_response(self) -> RayriftWithStreamedResponse:
        return RayriftWithStreamedResponse(self)
...
class AsyncRayrift(AsyncAPIClient):
    # client options
    api_key: str
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncRayrift client instance.
    ...
    @cached_property
    def with_raw_response(self) -> AsyncRayriftWithRawResponse:
        return AsyncRayriftWithRawResponse(self)

    @cached_property
    def with_streaming_response(self) -> AsyncRayriftWithStreamedResponse:
        return AsyncRayriftWithStreamedResponse(self)
...
class RayriftWithRawResponse:
    _client: Rayrift

    def __init__(self, client: Rayrift) -> None:
        self._client = client
...
class AsyncRayriftWithRawResponse:
    _client: AsyncRayrift

    def __init__(self, client: AsyncRayrift) -> None:
        self._client = client
...
class RayriftWithStreamedResponse:
    _client: Rayrift
...
class AsyncRayriftWithStreamedResponse:
    _client: AsyncRayrift
